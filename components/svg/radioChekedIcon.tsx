...
const RadioCheckedIcon: FC<Props> = (props) => {

  const { className, color, width, height } = props;

  return (
    <svg width={width ?? 29} height={height ?? 29} viewBox="0 0 29 29"
      className={clsx('radio-checked-icon', { [className ?? '']: className })}
    >
      <g transform="translate(0.399 -18.02)">
        ...
        <circle cx="8.5" cy="8.5" r="8.5" transform="translate(5.601 24.02)" fill={color ?? "#9f83eb"} />
      </g>
    </svg>
  )
}
